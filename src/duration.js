 ...
 *
 * - If the input is not a number or less than 0, returns the default duration.
 * - If the input is 0 or exceeds the maximum, returns the maximum duration.
...
function getDuration(expectedDuration) {
  if (typeof expectedDuration !== "number" || expectedDuration < 0) {
    return DURATION;
  } else if (expectedDuration === 0 || expectedDuration > MAX_DURATION) {
    return MAX_DURATION;
  }
